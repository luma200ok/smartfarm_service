...
import type { ReadingCellState, ReadingMatrixResponse, SensorMetric, ZoneTreeResponse } from "@/types";
...
export default function FarmRackPanel({ farmId, farmName }: FarmRackPanelProps) {
  ...
  const { cells, columns, cellMeta } = useMemo(() => {
    if (!matrix || matrix.racks.length === 0) {
      return { cells: [] as CellState[][], columns: [] as string[], cellMeta: [] as RackCellMeta[][] };
    }
  ...
  const selectedMeta = selectedCell ? cellMeta[selectedCell.row]?.[selectedCell.col] : null;
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex max-h-[280px] min-h-[180px] flex-1 gap-3 overflow-x-auto">
            ...
            <RackGrid
              ...
              onCellDoubleClick={() => router.push(`/farms/${farmId}/control`)}
            />
          </div>
            ...
            {selectedMeta && (
              <>
                ...
                <span className="text-[11.5px] leading-none font-semibold text-dp-green-ink">
                  {selectedMeta.rackCode}랙 {selectedMeta.levelNo}층 선택됨 · {SENSOR_METRIC_LABELS[metric]}{" "}
                  {selectedMeta.value === null ? "측정 없음" : selectedMeta.value.toFixed(1)} ·{" "}
                  {READING_CELL_STATE_LABELS[selectedMeta.state]} · 더블클릭하면 제어 화면으로 이동
                </span>
              </>
            )}
          </div>
