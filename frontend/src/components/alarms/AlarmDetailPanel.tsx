...
import { alarmDisplayTone, alarmToneTextClass, formatAlarmDateTime, formatAlarmTimestamp } from "@/lib/alarmDisplay";
...
export default function AlarmDetailPanel({
  ...
  if (!detail) {
    return (
      <Card className="px-4 py-[15px]">
        <p className="text-[12.5px] text-dp-muted">왼쪽 목록에서 알람을 선택하면 상세 정보가 표시됩니다.</p>
      </Card>
    );
  }
  ...
  const severityLabel = tone === "done" ? "완료" : event.severity === "CRITICAL" ? "경보" : "주의";
    ...
    <div className="flex flex-col gap-3">
      <Card className="px-4 py-[15px]">
        <div className="mb-3 flex items-baseline justify-between gap-2">
          ...
          <span className={`flex-none font-mono text-[10.5px] leading-none font-semibold ${alarmToneTextClass(tone)}`}>
            {severityLabel}
          </span>
        </div>
        ...
        <dl className="flex flex-col gap-[7px] text-[12px] leading-[1.5] text-dp-body">
          ...
          {!ruleLoading && ruleSummary && <Field label="규칙">{ruleSummary}</Field>}
          <Field label="발생">{formatAlarmDateTime(event.occurredAt)}</Field>
          <Field label="상태">{ALARM_STATUS_LABELS[event.status]}</Field>
        </dl>

        {actionError && <p className="mt-2 text-[11.5px] text-dp-red-ink">{actionError}</p>}
        ...
        <Link
          ...
          className="mt-2 block rounded-[7px] bg-dp-green py-2.5 text-center text-[12.5px] leading-none font-semibold text-dp-on-green"
        >
          제어 화면 이동
        </Link>
      </Card>
      ...
      <Card className="flex flex-1 flex-col px-4 py-[15px]">
        ...
        {canWrite && (
          <button
            type="button"
            onClick={onMemoOpen}
            className="mt-2.5 rounded-[7px] border border-dp-line-strong py-2.5 text-center text-[12.5px] leading-none font-semibold text-dp-body hover:bg-dp-inset"
          >
            메모 남기기
          </button>
        )}
...
function TimelineRow({ time, text, last }: { time: string; text: string; last: boolean }) {
  return (
    <div className={`flex gap-2.5 py-[7px] ${last ? "" : "border-b border-dp-line-row"}`}>
      ...
      <span className="flex-1 text-[12px] leading-[1.4] font-medium text-dp-ink">{text}</span>
    </div>
  );
}
