...
| # | 영역 | 상태 | 규모 | 비고 |
|---|---|---|---|---|
| 1 | **알람 이벤트** | ❌ 통째로 없음 | **큼** | 이벤트 저장·등급(경보/주의/완료)·확인/처리 전이·처리 이력 타임라인·7일 통계 |
| 2 | **알람 규칙 확장** | 🟡 4개 컬럼뿐 | 중 | 현재 `farm_env_thresholds`는 `indoorTemp/Humidity min/max`만. 프리뷰 규칙(`급액 EC > 2.8 dS/m · 5분 지속`)이 요구하는 **지표 7종·지속시간·랙 단위 스코프 없음** |
...
⚠️ **①이 최우선이다** — 프리뷰 알람 화면뿐 아니라 **TopBar의 "미확인 알람 N건" 배지가 여기 의존**한다. 진짜 새 디자인 셸을 만들려면 선행돼야 한다.
