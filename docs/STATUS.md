...
| 6 | 시스템 로그 | 🔄 **#129 묶음 진행 중** | 소 | 관리 화면. append-only, 기록 지점 4곳으로 제한(펌웨어 배포 등 아직 없는 기능은 제외) |
| 7 | 오늘 할일 · 브리핑 pill | 🔄 **#129 묶음 진행 중** | 소 | ⚠️ **실측(2026-08-26)**: 3개 중 2개는 **이미 있다**(조치필요=`unacknowledged-count` #116 · 보정임박=`DeviceSummaryResponse.calibrationDueSoon`). **수확예정만 새 도메인 필요** → #130 분리(랙에 작물·정식일 필드가 없고 `GrowthStage`는 날짜 개념이 아니다). 없는 값을 0으로 내보내면 거짓 정보라 **필드 자체를 만들지 않음** |
| 8 | 스케줄·자동화 규칙 | 🔄 **#129 묶음 진행 중** | — | 프리뷰에 **화면 자체가 없다**(`mock.ts:21` "대표 화면 외 디자인 미정"). **데이터모델·CRUD 골격만**(사용자 결정) — 실행 경로는 만들지 않는다 |
...
### 5. 다음 순서 제안
- **현재 진행 중(2026-08-24~)**: 미구현 백엔드 도메인 8건을 순차 처리 중. ①알람 이벤트 ✅완료(#116/PR #117) → ②알람 규칙 확장 ✅완료(#118/PR #121) → ③권한 4단계 ✅완료(#122·#123/PR #125) → ④CSV·저장한 분석 ✅완료(#126/PR #127) → ⑤농약 참조정보 ✅완료(#128/PR #131) → **⑥⑦⑧ #129 묶음 진행 중**(속도 우선으로 3건 1사이클·리뷰 1회 — 사용자 결정 2026-08-26) → ④리포트·CSV → ⑤농약 참조정보(목데이터/스텁 — 사용자 결정) → ⑥시스템 로그 → ⑦오늘 할일 → ⑧스케줄·자동화(백엔드 데이터모델·CRUD 골격만 — 사용자 결정)
