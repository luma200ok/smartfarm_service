...
| 4 | 리포트·CSV 내보내기·저장한 분석 | ❌ — **다음 차례** | 중 | 데이터 화면 하단. **범위 결정(2026-08-25)**: CSV 내보내기 + 저장한 분석 CRUD만. PDF/XLSX 리포트·예약은 새 의존성(iText/POI)+스케줄러가 필요해 후속 분리 |
| 5 | 농약 참조정보 | ❌ | 소 | 농진청 연동 성격. 진단/처방과 용도 다름 |
...
### 5. 다음 순서 제안
- **현재 진행 중(2026-08-24~)**: 미구현 백엔드 도메인 8건을 순차 처리 중. ①알람 이벤트 ✅완료(#116/PR #117) → ②알람 규칙 확장 ✅완료(#118/PR #121) → ③권한 4단계 ✅완료(#122·#123/PR #125) → **④리포트·CSV 착수 예정** → ④리포트·CSV → ⑤농약 참조정보(목데이터/스텁 — 사용자 결정) → ⑥시스템 로그 → ⑦오늘 할일 → ⑧스케줄·자동화(백엔드 데이터모델·CRUD 골격만 — 사용자 결정)
