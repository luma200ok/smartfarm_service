...
- **미구현 도메인 정리 (2026-08-24~)**: [x] **#116 [BE] 알람 이벤트 도메인** (**PR #117 머지 2026-08-24** — V19 `alarm_events`/`alarm_event_logs`, 상태전이 API 8종, 임계치 훅 연동. Critical 사이클: code-reviewer opus P1 2건 + 재검토에서 **픽스 유발 회귀 2건**(P1급 상향) + P2 3건 전량 픽스, security-reviewer opus P1 0건. 테스트 469→564. 계약 §4.13 신설·§4.6 갱신) · [ ] #118 [BE] 알람 규칙 확장(지표 7종·지속시간·랙 스코프·등급 분화) — **다음 차례**
...
| 2 | **알람 규칙 확장** | 🟡 4개 컬럼뿐 — **#118 등록·다음 차례** | 중 | 현재 `farm_env_thresholds`는 `indoorTemp/Humidity min/max`만. 프리뷰 규칙(`급액 EC > 2.8 dS/m · 5분 지속`)이 요구하는 **지표 7종·지속시간·랙 단위 스코프 없음** |
| 3 | **권한 4단계** | 🟡 2단계 | 중 | `FarmRole`=OWNER/MEMBER → 관리자/제어가능/조회전용/대기 |
...
⚠️ **③은 단순 enum 추가가 아니다** — 제어 도메인(#100)이 이미 OWNER/MEMBER 기준으로 권한을 박아놨다(비상 정지 OWNER 전용 등). "제어가능"이 생기면 **그 판정을 전부 재검토**해야 한다.
...
### 5. 다음 순서 제안
- **현재 진행 중(2026-08-24~)**: 미구현 백엔드 도메인 8건을 순차 처리 중. ①알람 이벤트 ✅완료(#116/PR #117) → **②알람 규칙 확장(#118) 착수 예정** → ③권한 4단계 → ④리포트·CSV → ⑤농약 참조정보(목데이터/스텁 — 사용자 결정) → ⑥시스템 로그 → ⑦오늘 할일 → ⑧스케줄·자동화(백엔드 데이터모델·CRUD 골격만 — 사용자 결정)
