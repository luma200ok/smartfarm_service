...
- **자원 보호(2026-08-23 보안 리뷰 P2)**: ai-server는 챗 전용 하위 상한 1(진단·처방이 굶지 않도록 최소 1슬롯 확보) — 챗 포화 시 CH002. backend는 추가로 **농장 단위** 레이트리밋(분당 10건, 인메모리 고정 윈도 — 단일 인스턴스 전제)을 둔다. 사용자 단위가 아니라 농장 단위인 이유: 데모 농장처럼 여러 사용자가 공유하는 농장의 합산 남용을 막는 게 목적이고, 데모 계정은 농장 생성이 차단(§4.5)돼 농장을 늘려 우회할 수 없다.
