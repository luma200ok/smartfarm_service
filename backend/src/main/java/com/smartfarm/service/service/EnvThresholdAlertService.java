...
public class EnvThresholdAlertService {
     ...
     *
     * <p>⚠️ <b>알려진 공백 — 이 규칙은 현재 사실상 수동 토글 전용이다(후속 이슈, #118 리뷰 P2-4).</b>
     * 판정 신호가 둘인데 둘 다 자동으로 갱신되지 않는다:
     * <ul>
     *   <li>{@code lastSeenAt}은 §4.10 모델에 컬럼만 있고 <b>쓰는 곳이 없다</b>. 그래서
     *       {@code lastSeenAt == null}은 부재로 치지 않는다 — 그러지 않으면 전 장비가 상시
     *       발동한다. 값이 채워지기 시작하면 이 판정이 저절로 살아난다.</li>
     *   <li>{@link DeviceStatus#OFFLINE}은 계약 §4.10이 정의한 통신 두절 상태이고 이 코드베이스에서
     *       "관리되는" 유일한 신호지만, 그걸 <b>자동으로 세팅하는 경로도 없다</b> — 사용자의
     *       {@code PATCH /devices}가 유일한 writer다.</li>
     * </ul>
     * 즉 <b>진짜 하트비트 수신 기록이 도입되기 전까지</b> 이 규칙은 사람이 장비를 OFFLINE으로
     * 표시했을 때만 발동한다. 수신 기록 writer는 별도 기능이라 이 사이클에서 분리했다. 그때까지
     * 이 소스로 만든 규칙의 한계를 사용자에게 알려야 한다(PR·계약 고지 대상).
     *
     * <p>평가 로직 자체는 그 writer가 생기는 즉시 무변경으로 동작한다 — 아래 판정이 두 신호를
     * 이미 OR로 보고 있기 때문이다.
     *
