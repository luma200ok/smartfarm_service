 ...
 * 플래그가 그 판정 근거).
 *
 * <p><b>황산칼륨(K2SO4) 채택 근거 — contract §4.9 2026-08-23 정정</b>: 최초 code-reviewer는
 * "제1인산칼륨(MKP)만으로 못 채운 K 부족분을 질산칼륨(KNO3)으로 마저 채우면 안 되나"를 P1로
 * 지적했으나, 검토 결과 이 구현(K2SO4 사용)이 맞고 그 지적이 틀렸다고 판정해 계약을 정정했다.
 * 이유: Ca(NO3)2로 Ca를 맞춘 시점에 N이 이미 목표에 근접·초과한 조합(프리셋 4단계 전부 해당)이
 * 흔한데, 이 상태에서 남는 K를 KNO3로 채우면 N이 목표를 더 초과해 EC 상승·질산태 질소 과잉이라는
 * 직접적인 작물 피해로 이어진다. 반면 K2SO4가 함께 들이는 S(황)는 프리셋 출처(OSU HYG-1437)가
 * "10~200 mg/L 허용 범위"로 명시하고 있고, 본 구현의 4단계 프리셋 실측 S 공급량(62~141mg/L,
 * NutrientCalculationEngineTest 검산값)이 전부 그 범위 안이다 — 즉 K2SO4의 부산물(S)은 출처가
 * 명시적으로 허용하는 범위 내이지만 KNO3의 부산물(N 초과)은 그런 안전 여유가 없다. 그래서 K
 * 부족분은 항상 K2SO4로 채우고, KNO3는 N 부족분을 채울 때만 사용한다(현재 알고리즘 그대로 유지).
 *
