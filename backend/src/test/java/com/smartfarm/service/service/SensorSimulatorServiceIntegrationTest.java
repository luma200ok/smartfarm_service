 ...
 * {@code sensor-simulator.max-per-farm=3}으로 낮춰 소수 장비로 상한 초과 케이스를 재현한다.
...
class SensorSimulatorServiceIntegrationTest extends FarmTestSupport {
    ...
    @Test
    @DisplayName("tick: kind=SENSOR·status!=OFFLINE 장비마다 7종 지표 전부를 생성한다")
    void tickGeneratesAllMetricsForEligibleSensor() throws Exception {
        ...
        long deviceId = createDevice(ownerToken, farmId, zoneId, rackId, levelId, "센서1");
        ...
        assertThat(readings).hasSize(SensorMetric.values().length);
        readings.forEach(r -> {
    ...
    @Test
    @DisplayName("tick: 농장당 상한(테스트 설정 3) 초과 시 초과분은 생성하지 않는다(조용히 자르지 않고 WARN만)")
    @DirtiesContext
    void tickCapsGenerationPerFarm() throws Exception {
        String ownerToken = signupAndLogin("sim-owner4");
        long farmId = createFarm(ownerToken, "농장-시뮬4");
        long zoneId = createZone(ownerToken, farmId, "A동");
        List<Long> deviceIds = List.of(
                createDevice(ownerToken, farmId, zoneId, null, null, "센서1"),
                createDevice(ownerToken, farmId, zoneId, null, null, "센서2"),
                createDevice(ownerToken, farmId, zoneId, null, null, "센서3"),
                createDevice(ownerToken, farmId, zoneId, null, null, "센서4"),
                createDevice(ownerToken, farmId, zoneId, null, null, "센서5"));
        ...
        // 상한 3개 — id 오름차순으로 앞의 3개 장비만 생성되고 나머지 2개는 생성되지 않는다.
        assertThat(devicesWithReadings).isEqualTo(3);
    }
    ...
    private void setDeviceStatus(String ownerToken, long farmId, long deviceId, DeviceStatus newStatus)
        ...
        mockMvc.perform(MockMvcRequestBuilders.patch("/api/farms/" + farmId + "/devices/" + deviceId)
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new DeviceRequest(
                                null, null, null, null, null, null, null, newStatus, null, null))))
                .andExpect(status().isOk());
    }
}
